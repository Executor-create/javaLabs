...
/**
 * {@link CrazyLambdas} is an exercise class. Each method returns a functional
 * interface and it should be implemented
 * using either lambda or a method reference. Every method that is not
 * implemented yet throws
 * {@link ExerciseNotCompletedException}.
 * <p>
 * TODO: remove exception and implement each method of this class using lambda
 * or method reference
 * <p>
 * <p>
...
public class CrazyLambdas {
     ...
     * repeated n time, where n is passed
     ...
     * {@link Predicate<String>} that verifies if string
    ...
    /**
     * Receives a {@link Runnable} parameter, and returns a
     * {@link Supplier<Thread>}. The thread will be started only
    ...
    /**
     * Returns a {@link Consumer} that accepts {@link Runnable} as a parameter and
    ...
    /**
     * Returns a {@link Function} that accepts an instance of {@link Runnable} and
     * returns a {@link Supplier} of a
     ...
     * {@link IntUnaryOperator} which is some integer function.
     * Second is {@link IntPredicate} which is some integer condition. And the third
     * is {@link IntUnaryOperator} which is
     * a new composed function that uses provided predicate (second parameter of
     * binary function) to verify its input
     * parameter. If predicate returns {@code true} it applies a provided integer
     * function
     * (first parameter of binary function) and returns a result value, otherwise it
     * returns an element itself.
     *
     ...
     * is a function name, and value is some
     * {@link IntUnaryOperator}, and second parameter is a {@link String} which is a
     * function name. If the map contains a
    ...
    /**
     * Returns a comparator of type T that is comparing values extracted using the
     ...
     * }</pre>
     * <p>
     * PLEASE NOTE, that @{@link Comparator} is a functional interface, and you
     * should manually write a lambda expression
     * to implement it.
     *
     * @param mapper a mapper function that allows to map an object to a comparable
     *               value
    ...
    /**
     * Returns a comparator of type T that uses a provided comparator to compare
     * objects, and only if they are equal
     * it's comparing values extracted using the provided mapper function.
     * <p>
     * E.g. suppose you want to compare accounts by balance, but in case two people
     * have the same balance you want to
     ...
     * @param mapper     a mapper function that is used to extract values when
     *                   initial comparator returns zero
     * @return a comparator instance
     */
